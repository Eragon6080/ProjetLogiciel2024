...
CREATE TABLE Delivrable(
  idDelivrable SERIAL PRIMARY KEY,
...
CREATE TABLE Superviseur(
  idSuperviseur SERIAL PRIMARY KEY,
...
CREATE TABLE Supervision(
  idSupervision SERIAL PRIMARY KEY,
  description TEXT NOT NULL,
...
CREATE TRIGGER unique_teacher_or_supervisor_for_a_subject
  ...
  ON public.sujet
  FOR EACH ROW
    EXECUTE PROCEDURE check_idprof_idsuperviseur();

CREATE TRIGGER not_below_for_assignation_for_a_subject
  BEFORE INSERT OR UPDATE
  ON public.selectionsujet
  FOR EACH ROW
    EXECUTE PROCEDURE check_not_below_for_assignation_for_a_subject();

CREATE TRIGGER est_reserve
  BEFORE INSERT OR UPDATE
  ON public.selectionsujet
  FOR EACH ROW
    EXECUTE PROCEDURE set_reserve_to_true();
